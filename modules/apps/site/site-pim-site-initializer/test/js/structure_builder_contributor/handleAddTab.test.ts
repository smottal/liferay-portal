...
import handleAddTab from '../../../src/main/resources/META-INF/resources/js/structure_builder_contributor/handleAddTab';
...
describe('handleAddTab', () => {
	...
	it('toasts and does not dispatch when the parent is not the structure root', () => {
		const dispatch = jest.fn();
		const group = panel();
		...
		expect(openToast).toHaveBeenCalledWith({
			message: 'a-tab-can-only-be-created-at-the-top-level',
			type: 'danger',
		});
		expect(dispatch).not.toHaveBeenCalled();
	});

	it('dispatches add-tab and does not toast for a valid top-level selection', () => {
		const dispatch = jest.fn();
		const sku = field('sku');
