...
import {REP_GROUP_BOX_NAME_PREFIX} from './buildObjectLayout';
...
export default function applyObjectLayout({
	...
	const takeBoxItems = (
		...
		if (name.startsWith(REP_GROUP_BOX_NAME_PREFIX)) {
			const relationshipName = name.substring(
				REP_GROUP_BOX_NAME_PREFIX.length
			);
	...
	const buildTabChildren = (
		...
		for (const objectLayoutBox of objectLayoutTab.objectLayoutBoxes ?? []) {
			...
			const isPanel =
				...
				!name.startsWith(REP_GROUP_BOX_NAME_PREFIX);
