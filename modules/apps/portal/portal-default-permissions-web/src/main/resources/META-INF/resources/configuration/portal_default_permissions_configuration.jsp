...
>
	<clay:management-toolbar
		managementToolbarDisplayContext="<%= new PortalDefaultPermissionsManagementToolbarDisplayContext(request, liferayPortletRequest, liferayPortletResponse, portalDefaultPermissionsConfigurationDisplayContext.getSearchContainer()) %>"
	/>

	<div data-qa-id="portal-default-permissions-search-container">
		<liferay-ui:search-container
			id="portalDefaultPermissionsSearchContainer"
			searchContainer="<%= portalDefaultPermissionsConfigurationDisplayContext.getSearchContainer() %>"
		>
			...
			>
				...
				<liferay-ui:search-container-column-text>
					...
					<%
					List<DropdownItem> dropdownItems = portalDefaultPermissionsConfigurationDisplayContext.getActionDropdownItems(portalDefaultPermissionsSearchEntry);
					%>
						...
						<c:when test="<%= dropdownItems.size() > 1 %>">
							<div data-qa-id='<%= "actions-" + portalDefaultPermissionsSearchEntry.getLabel() %>'>
								<clay:dropdown-actions
									...
									propsTransformer="{portalDefaultPermissionsComponentPropsTransformer} from portal-defaultpermissions-web"
								/>
							</div>
						</c:when>
						<c:otherwise>
							...
							<clay:button
								...
								propsTransformer="{portalDefaultPermissionsButtonPropsTransformer} from portal-defaultpermissions-web"
								small="<%= true %>"
							/>
						</c:otherwise>
					</c:choose>
